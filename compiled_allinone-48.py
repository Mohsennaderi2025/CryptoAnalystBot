...
class DatabaseManager:
    ...
    async def log_trade(self, user_id, symbol, interval, signal_info):
        try:
            ...
            cursor.execute("""
                INSERT INTO trades
                (user_id, timestamp, symbol, interval, signal, price, tp, sl, size, confidence)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                user_id,
                datetime.now(),
...
class CryptoAnalyzer:
    ...
    def generate_signals(self, df, config):
        try:
            ...
            risk_amount = config["risk_management"]["max_risk_per_trade"] * 10000
            position_size = (
                min(
...
⏳ زمان تحلیل: {signal_info['timestamp']}"""
    ...
    async def group_analysis(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"🔎 در حال تحلیل 5 رمزارز برتر... لطفاً صبر کنید..."
        )

        top_symbols = await self.analyzer.fetch_top_symbols(limit=5)
        results = []
...
if __name__ == "__main__":
    ...
    try:
        ...
        error_logger.error(f"Application error: {e}")
        traceback.print_exc()